...
from components.utils import check_win
...
class Turn(object):
    ...
    def action(self, player, opponent, agent, algorithm, turn_no):
        """
        ...
        if len(player_act.hand_play) > 0:
            ...
            if player_act == self.player_1:
                player_act.play_agent(self.deck, self.card_open, agent, algorithm, turn_no)
            else:
