import random
from components.utils import underline
...
class Player(object):
    ...
    def identify_state(self, card_open):
        ...
        for key, val in zip(norm_cards.keys(), norm_cards.values()):
            self.state[key] = min(
                [1 if (card.color == key) and (card.value in range(0, 10)) else 0 for card in self.hand].count(1), val)
        ...
        for key, val in zip(norm_cards.keys(), norm_cards.values()):
            self.state[key + "#"] = min(
                [1 if (card.color == key) and (card.value in range(0, 10)) else 0 for card in self.hand_play].count(1),
                val - 1)
    ...
    def play_agent(self, deck, card_open, agent, algorithm, turn_no):
        """
        ...
        if algorithm == "q-learning":
            agent.update(self.state, self.action, turn_no)
